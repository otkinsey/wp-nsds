...
function frontier_user_posts($atts)
	...
	if ( has_shortcode( $post->post_content, 'frontier-post') && ( in_array($post->post_type, $sc_allowed_post_types) ) )
		...
		if( is_user_logged_in() )
			...
			if ( isset($_GET['task']) && $_GET['task'] == "new" && isset($_GET['frontier_add_post_type']) )
				{
				...
				if ( in_array($tmp_post_type , $tmp_post_type_list) )
					$frontier_post_shortcode_parms['frontier_add_post_type'] = $tmp_post_type;
				}
			...
			$frontier_post_shortcode_parms['frontier_user_status'] = fp_list2array($frontier_post_shortcode_parms['frontier_user_status']);
			...
			switch( $post_task )
				...
				case 'approve':
					if ( $post_action == "wpfrtp_approve_post" )
						frontier_execute_approve_post($frontier_post_shortcode_parms);
					else
						frontier_prepare_approve_post($frontier_post_shortcode_parms);
					break;
...
$ns_blog_user = get_nsblog_user($_POST['Username']);

function get_nsblog_user($username){
	global $wpdb;

		$query = $wpdb->get_row("SELECT * FROM ns_users WHERE user_login = '$username' ");

	return $query;
}

function set_blog_session($variable){
 global $wpdb;

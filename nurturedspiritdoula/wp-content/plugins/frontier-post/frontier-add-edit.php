...
function frontier_post_add_edit($frontier_post_shortcode_parms = array())
	...
	if (!is_user_logged_in())
		...
		if(isset($_REQUEST['task']) && $_REQUEST['task']=="edit")
			...
			{
			$tmp_post_type	= post_type_exists($frontier_add_post_type) ? $frontier_add_post_type : 'NONE';
			//echo "Post type: ".$tmp_post_type."<br>";
			if ( frontier_can_add($tmp_post_type) == true )
				...
				{
				echo '<br><div id="frontier-post-alert">'.__("You are trying to add a post with an illegal post-type", "frontier-post").': '.$tmp_post_type.'</div><br>';
				return;
				}
			}

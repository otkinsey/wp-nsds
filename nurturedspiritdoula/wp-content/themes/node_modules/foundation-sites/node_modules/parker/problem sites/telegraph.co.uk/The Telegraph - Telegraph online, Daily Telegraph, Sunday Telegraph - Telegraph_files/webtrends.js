...
(function (_window, _document, _navigator, _location) {
	...
	Webtrends = { //changed by TMG from var Webtrends = {
		dcss: {},
		plugins: {},
		dcssIdx: 0,
		gWtId: {},
		...
		*/
		...
		dcsVar: function () {
			...
			if(_location.hash){
				...
				DCS["source"]=_location.hash.substring(8,15);		
			}
        ...
        //function added by TMG
        dcsMetaCustom: function (){
            var DCSext = this.DCSext;
            var isEmpty = this.isEmpty;
            var tmgWebTrendsCookie = this.dcsGetCookie("tmg_web_trends");
            ...
            for (var i=0;i<tmgWebTrendsCookieVals.length;i++){
                if(tmgWebTrendsCookieVals[i].slice(0, 19) == "meter_viewed_count:"){
                    DCSext["cn"]=tmgWebTrendsCookieVals[i].substring(19);
                }else if(tmgWebTrendsCookieVals[i].slice(0, 14) == "meter_updated:"){
                    if(isEmpty(DCSext["cd"])){
                        DCSext["cd"]=tmgWebTrendsCookieVals[i].substring(14);
                    }
                }
            }
            //document.cookie = "tmg_web_trends=; path=/; domain=.telegraph.co.uk; expires=Thu, 01-Jan-70 00:00:01 GMT";
        },
		dcsTag: function (o) {
			if (_document.cookie.indexOf("WTLOPTOUT=") != -1) {
				return;
		...
		pageAnalyze: function () {
			this.dcsVar();
			this.dcsMeta();
			this.dcsMetaCustom(); //added by TMG
